...
When writing a story anonymously, a unique UUID token (`submitter_token`) is stored in the database and provided to the client. This token acts as a password key, letting the author modify or remove their post directly without registering an account.
