// permissions.js — RBAC permission engine for the admin worker

/**
 * Calculates effective permissions for an employee.
 *
 * Rule:
 *   effective = (role permissions via team)
...
export function requirePermission(code) {
  return async (c, next) => {
    ...
    // Super Admin (admin_users with role=superadmin) bypass all checks
...
export async function resolveTicketRouting(db, accountId, categoryId, subcategoryId) {
  ...
  // 3. Fallback: escalation queue (team with no account_id)
